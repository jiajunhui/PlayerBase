...
public class TestKeyActionDownUpCover extends BaseFocusCover {

    public TestKeyActionDownUpCover(Context context, BaseCoverObserver coverObserver) {
    ...
    public void onCoverEvent(int eventCode, Bundle bundle) {
        ...
        switch (eventCode){
            case IDpadFocusCover.EVENT_CODE_ACTION_DOWN_UP:
                requestFocus();
                setCoverState(true);
                break;
        }
    ...
    public View initCoverLayout(Context context) {
        return View.inflate(context, R.layout.layout_test_onkey_down_up_cover,null);
    }
}
